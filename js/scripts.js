...
const DOMFuncs = (function(){
	...
	function _obtainFreeSquares(){
		...
		return arr;
				...
				* to mark that square.
				**/
	function _markAiSquare(idx){
		for(square of squares){
			if(square.id[square.id.length - 1] == idx){
				square.click();
			}
		}
	}
	...
	function _easyDifficulty(){
		...
		let squareIdx = _generateRandomNum(squaresArr.length - 1);
		_markAiSquare(squaresArr[squareIdx]);
	}
