...
const DOMFuncs = (function(){
	...
	function _invokeAI(){
		...
		**/
		...
		inputEl.placeholder = gameFlow.ai.name;
				...
				**/
	function _aiMove(){
		if(gameFlow.ai.difficulty == "easy" && gameFlow.ai.turn){
			_easyDifficulty();
			console.log("_aiMove")
		}
	}
		...
		// --v-- Shared by the three levels of difficulty.
				/**
				* Function.
				* It returns an array containing all the empty squares from the
				* gameFlow.board obj.
				**/
	function _obtainFreeSquares(){
		...
		return arr;
				...
				/**
				* Function.
				* This function takes an argument "square". It will take an int
				* that is gonna use to search for the DOM element corresponding to that square
				* in the nodelist "squares", once it find the corresponding element execute a click
				* to mark that square.
				**/
	function _markAiSquare(square){
		...
		// --^-- Shared by the three levels of difficulty
				...
				/**
				* Function.
				* It takes one parameter "max" that is gonna be the length - 1 of the array generated by
				* _obtainFreeSquare. It generates a random int number that is gonna be used tu access such array.
				**/
	...
	function _easyDifficulty(){
		let squaresArr = _obtainFreeSquares();
		let squareIdx = _randomNumber(squaresArr.length - 1);
		_markAiSquare(squareIdx);
	}
